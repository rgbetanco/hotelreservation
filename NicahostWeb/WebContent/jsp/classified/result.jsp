	...
	<table border="0" cellspacing="0" cellpadding="0" width="100%">
	  ...
	  <tr>	  
	    <td colspan="4" align="center"> 
	    	<table border="0" width="550" cellpadding="5" cellspacing="1"  id="sessionTable">
		        <tr>
		          <th colspan="3" align="left">
	    	        <fmt:message key="label.classified.totalHits" /> <b><c:out value="${totalHits}" /></b>
	        	  </th>
   			...
   			<c:forEach items="${result}" var="adBean">
   				<tr>
   					<td colspan="4">
   				...
   				<tr>
   					...
   					<c:choose>
   						<c:when test="${hasPrevious}" >
   							<a href="/NicahostWeb/clasificados/browse.do?criteria=<c:out value='${criteria}'/>&page=<c:out value='${currentPage - 1}'/>"> &lt; <fmt:message key="label.classified.previous" /></a>
   							&nbsp;
   						</c:when>
   						<c:otherwise>   						
						...
						%>						
							<a href="/NicahostWeb/clasificados/browse.do?criteria=<c:out value='${criteria}'/>&page=<%=i%>">
						<%	
								if (currentPage == i) {
						%>
									<font color="red">
   					...
   					<c:choose>
   						<c:when test="${hasNext}" >
   							&nbsp; <a href="/NicahostWeb/clasificados/browse.do?criteria=<c:out value='${criteria}'/>&page=<c:out value='${currentPage + 1}'/>"><fmt:message key="label.classified.next" /> &gt;</a>
   						</c:when>
   						<c:otherwise>
